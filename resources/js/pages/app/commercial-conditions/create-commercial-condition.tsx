...
export default function CreateCommercialCondition({ customers, regions, campaigns, establishmentTypes }: any) {
    ...
    const submit = (event: any) => {
        ...
        post(route('app.commercial-conditions.store'), { onSuccess: () => reset() });
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Nova condição comercial" />
            <div className="flex min-h-16 flex-col justify-center gap-1 px-4 py-3">
                <div className="flex items-center gap-2">
                    <HandCoins className="h-8 w-8" />
                    <h2 className="text-xl font-semibold tracking-tight">Nova condição comercial</h2>
                </div>
            </div>
