...
export default function EditCampaign({ campaign, products, regions, brands, categories }: any) {
    const { data, setData, patch, processing, errors } = useForm({
        name: campaign.name ?? '',
        audience_type: campaign.audience_type ?? (campaign.region_id ? 'region' : 'all'),
        scope_type: campaign.scope_type ?? 'product',
        product_ids: campaign.products?.map((product: any) => product.id) ?? [],
        region_id: campaign.region_id ?? '',
        brand: campaign.brand ?? '',
        category: campaign.category ?? '',
        starts_at: dateValue(campaign.starts_at),
        ends_at: dateValue(campaign.ends_at),
        goal: campaign.goal ?? '',
    ...
    const submit = (event: any) => {
        ...
        patch(route('app.campaigns.update', campaign.id));
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex min-h-16 flex-col justify-center gap-1 px-4 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-semibold tracking-tight">Editar campanha</h2>
                </div>
            </div>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="grid gap-2">
                                ...
                                <select id="audience_type" className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs outline-none md:text-sm" value={data.audience_type} onChange={(event) => setData((current) => ({ ...current, audience_type: event.target.value, region_id: '' }))}>
                                    {audienceTypes.map((audience) => <option key={audience.value} value={audience.value}>{audience.label}</option>)}
                                </select>
