...
export default function EditOrder({ order, customers, products, flex }: any) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        items: order.order_items.map((item: any) => ({ product_id: Number(item.product_id), quantity: Number(item.quantity) })),
    ...
    const addProduct = () => {
        ...
        setData('items', current
            ? data.items.map((item: any) => item.product_id === id ? { ...item, quantity: item.quantity + amount } : item)
            : [...data.items, { product_id: id, quantity: amount }]);
        setProductId('');
        setProductSearch('');
        setQuantity('1');
    };
