...
export default function EditOrder({ order, customers, products, flex }: any) {
    ...
    const rows = useMemo(() => data.items.map((item: any) => {
        ...
        return { ...item, product, price, total: price * item.quantity * (1 - Number(item.discount_percentage ?? 0) / 100) };
    ...
    const addProduct = () => {
        ...
        const discountPercentage = Math.min(Math.max(Number(itemDiscount), 0), 100);
        if (!id) return;
        const current = data.items.find((item: any) => item.product_id === id);
        setData('items', current
            ? data.items.map((item: any) => item.product_id === id ? { ...item, quantity: item.quantity + amount, discount_percentage: discountPercentage } : item)
            : [...data.items, { product_id: id, quantity: amount, discount_percentage: discountPercentage }]);
        setProductId('');
        setProductSearch('');
        setQuantity('1');
        setItemDiscount('0');
